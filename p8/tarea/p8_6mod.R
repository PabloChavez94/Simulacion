inicio <- Sys.time()
suppressMessages(library(testit)) # para pruebas
...
romperse <- function(tam, cuantos) {
  ...
  stopifnot(sum(resultado) == tam * cuantos) # no hubo perdidas, assert cambiado por stopifnot
  return(resultado)
}
...
syr <- function(){
  cumulos <- juntarse[2*i-1] + juntarse[2*i]
  return(cumulos)
}
...
for (paso in 1:duracion) {
  ...
  assert(sum(cumulos) == n)
  cumulos <- foreach(i = 1:dim(freq)[1], .combine = c) %dopar% separar()
  ...
  cumulos <- foreach(i=1:dim(freq)[1], .combine = c) %dopar% reunir ()
  ...
  if (nt > 0) {
    if (nt > 1) {
      ...
      cumuloss <- foreach(i = 1:floor(nt/2), .combine = c) %dopar% syr()
      cumulos <- c(positivos, cumuloss)
    }
    if (nt %% 2 == 1) {
      cumulos <- c(cumuloss, positivos, juntarse[nt])
    }
  }
  assert(sum(cumulos) == n)
  freq <- as.data.frame(table(cumulos))
  ...
  png(paste("p8m_ct", tl, ".png", sep = ""), width = 300, height = 300)
